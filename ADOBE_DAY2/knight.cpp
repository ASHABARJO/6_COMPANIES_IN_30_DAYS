...
public:
    ...
    double dfs(int i,int j,int n,int moves){
        ...
        if(dp[i][j][moves]!=0)
        return dp[i][j][moves];
        //find total possible ways of staying on chess
       double ans=0;
       for(int k=0;k<8;k++)
       {
           int new_row=i+dx[k];
           ...
           //for each cell there are 8 possible moves so probality will be
           //no. of suucessfully moves 8
           //store the result ans return
